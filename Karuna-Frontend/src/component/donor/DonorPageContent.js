...
const DonorPageContent = () => {
  ...
  const donateToReceiver = (event) => {
    ...
    let receiverId = localStorage.getItem("rec");
    DonorService.donateToReceiver(userId, receiverId, item)
    .then((response) => {
      ...
      <MDBContainer fluid className='d-flex align-items-center justify-content-center bg-image' style={{backgroundImage: 'url()'}}>
        ...
        <MDBCard className='m-5' style={{maxWidth: '600px'}}>
          <MDBCardBody className='px-5'>
            ...
            <MDBInput wrapperClass='mb-4' placeholder='quantity' size='lg' id='quantity' type='text' onChange={(e) => handleChange(e, "quantity")} value={item.quantity} />
            ...
            <Button className='mb-4 w-100 gradient-custom-4' size='lg' variant="outline-info" onClick={donateToReceiver}>Donate</Button>
          </MDBCardBody>
        </MDBCard>
      </MDBContainer>
    </div>
